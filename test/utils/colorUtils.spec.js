...
test('Color Utils =>', (expect) => {
  expect.test('::stringToHex', (expect) => {
    ...
    expect.equal(
      colorInt,
      16777215,
      'Should convert hex color string to color int',
    );
    expect.end();
  });
  ...
  expect.test('::hexToRGBA', (expect) => {
    ...
    expect.deepEqual(
      hexColor,
      [255, 255, 255, 255],
      'Should convert hex color to rgba color',
    );
    expect.end();
  });
  ...
  expect.test('::getPixelFromImageData', (expect) => {
    const context1 = new RenderingContext2d(),
      pixelIndex = getPixelFromImageData(
        context1.getImageData(0, 0, 100, 100),
        20,
        20,
      );
  ...
  expect.test('::putColor', (expect) => {
    ...
    expect.ok(
      iData.data.every((el) => el === 255),
      'Should color one pixel in the image data',
    );
    expect.end();
  });
  ...
  expect.test('::fillRectImageData', (expect) => {
    ...
    expect.deepEqual(
      getColor(iData, index1),
      color,
      'Should call putColor on image data',
    );
    expect.deepEqual(
      getColor(iData, index2),
      color,
      'Should fill specified rect on image data',
    );
    fillRectImageData(iData, 1, 1, 1, 1);
    expect.deepEqual(
      getColor(iData, index1),
      [0, 0, 0, 0],
      'Should remove color on index if no color provided',
    );
    expect.end();
  });
  ...
  expect.test('::equallyColored', (expect) => {
    ...
    expect.notOk(
      areEquallyColored,
      'Should return if pixelIndex is out of image data bounds',
    );
    areEquallyColored = equallyColored(imageData, 16, [0, 0, 0, 0]);
    expect.ok(
      areEquallyColored,
      'Should tell if pixel with index provided is of color as provided',
    );
    expect.end();
  });
  ...
  expect.test('::getBlackWhiteContrastColor', (expect) => {
    ...
    expect.equal(
      white,
      '#ffffff',
      'Should return white for dark color provided',
    );
    expect.equal(
      black,
      '#000000',
      'Should return black for light color provided',
    );
    expect.end();
  });
  ...
  expect.test('::getImageDataActiveColors', (expect) => {
    ...
    expect.ok(
      list.includes('#000000') && list.includes('#ffffff'),
      'Should create list of used colors present on the image data provided',
    );
    expect.end();
  });
  ...
  expect.test('::getAllActiveColors', (expect) => {
    ...
    expect.deepEqual(
      list,
      ['#000000', '#ffffff'],
      'Should create list of used colors from all image data present in the project',
    );
    expect.end();
  });
