...
test('Canvas Utils =>', (expect) => {
  expect.test('::enableImageSmoothing', (expect) => {
    ...
    expect.true(
      ...
      'Should apply image smoothing to context',
    );
    expect.end();
  });
  ...
  expect.test('::disableImageSmoothing', (expect) => {
    ...
    expect.false(
      context.imageSmoothingEnabled,
      'Should remove image smoothing from context',
    );
    expect.end();
  });
  ...
  expect.test('::drawGrid', (expect) => {
    ...
    expect.ok(
      ...
      'Should clear canvas before drawing grid',
    );
    expect.equal(
      context.lineTo.callCount,
      40,
      'Should create vertical and horizontal lines',
    );
    expect.ok(
      context.lineWidth === gutter && context.strokeStyle === stroke,
      'Should set grid options',
    );
  ...
  expect.test('::createCanvas', (expect) => {
    ...
    expect.ok(
      ...
      'Should create a canvas with width and height specified',
    );
    expect.end();
  });
  ...
  expect.test('::resizeImageData', (expect) => {
    ...
    expect.ok(
      iData.width === 100 && iData.height === 100,
      'Should return new image data of requested size',
    );
    expect.end();
  });
  ...
  expect.test('::extendImageData', (expect) => {
    ...
    expect.deepEqual(
      getColor(_iData, 0),
      [0, 0, 0, 0],
      'Should not touch other pixel than the central one',
    );
    expect.deepEqual(
      getColor(_iData, index),
      [255, 255, 255, 255],
      'Should not touch other pixel than the central one',
    );
    expect.end();
  ...
  expect.test('::combineImageDataToCanvas', (expect) => {
    ...
    expect.ok(
      ...
      'Should create a resulting canvas of proper size',
    );
    expect.end();
  });
