...
test('StateConverter =>', (expect) => {
  expect.test('::convertToExport', (expect) => {
    ...
    expect.ok(
      newState.meta.schemaVersion && newState.app,
      'Sould create meta and app entries in converted state',
    );
    expect.deepEqual(
      newState.app,
      converted,
      'Should convert state to a serializable',
    );
    expect.end();
  });
  ...
  expect.test('::convertToImport', (expect) => {
    ...
    expect.deepEqual(
      newState,
      state,
      'Should deserialize converted state to aplicable to redux',
    );
    expect.end();
  });
  ...
  expect.test('::createStateFromFramesData', (expect) => {
    ...
    expect.deepEqual(
      newState,
      state,
      'Should convert frame data to a state aplicable to redux',
    );
    expect.end();
  });
