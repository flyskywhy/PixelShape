...
test('framesCollection =>', (expect) => {
  ...
  expect.test('::addFrame', (expect) => {
    ...
    expect.equal(
      next[action.id].name,
      defaults.frameName + action.id,
      'Should add name to a new frame',
    );
    expect.ok(
      next[action.id].naturalImageData,
      'Should create image data object for a new frame',
    );
    expect.end();
  });
  ...
  expect.test('::updateFrameImageData', (expect) => {
    ...
    expect.deepEqual(
      next[uuid].naturalImageData,
      new ImageData(5, 5),
      'Should update frames image data',
    );
    expect.end();
  });
  ...
  expect.test('::duplicateFrame', (expect) => {
    ...
    expect.deepEqual(
      next[uuid].naturalImageData,
      next[action.uuid].naturalImageData,
      'Should duplicate image data to construct a new frame',
    );
    expect.end();
  });
  ...
  expect.test('::updateFrameName', (expect) => {
    ...
    expect.equal(
      next[uuid].name,
      'newborn',
      'Should change name of selected frame',
    );
    expect.end();
  });
  ...
  expect.test('::resetFramesState', (expect) => {
    ...
    expect.deepEqual(
      next,
      initialState,
      'Should completely reset collection to initial',
    );
    expect.end();
  });
