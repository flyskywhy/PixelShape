...
test('Boundshape =>', (expect) => {
  ...
  expect.test('::onMouseDown', (expect) => {
    ...
    expect.ok(
      boundshape.coords.x0 === 100 && boundshape.coords.y0 === 100,
  ...
  expect.test('::onMouseMove', (expect) => {
    ...
    expect.notOk(
      boundshape.update.called,
      'Should toggle drawing mode for rendering context',
    );

    boundshape.mouseDown = true;
    boundshape.onMouseMove(100, 100);
    expect.ok(
      boundshape._buffer.clearRect.called,
      'Should clear buffer context on each mouse move',
    );
    expect.ok(
      boundshape.update.calledWith(boundshape._buffer),
      'Should redraw shape on buffer on each mouse move',
    );
    expect.end();
  });
  ...
  expect.test('::onMouseUp', (expect) => {
    ...
    expect.ok(
      boundshape.update.calledWith(boundshape._ctx) &&
        boundshape._buffer.clearRect.called,
      'Should do final draw on rendering context and reset buffer context',
    );
    expect.end();
  });
  ...
  expect.test('::update', (expect) => {
    ...
    expect.ok(
      boundshape.draw.calledWith(ctx),
      'Should call draw method of tool on provided context',
    );
    expect.end();
  });
