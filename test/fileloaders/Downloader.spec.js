...
test('Downloader =>', (expect) => {
  expect.test('::prepareGIFBlobAsync', (expect) => {
    return Downloader.prepareGIFBlobAsync('abcd', 'newfile.gif').then(
      (dataObject) => {
        ...
        expect.equal(
          dataObject.name,
          'newfile.gif',
          'Should create object with specific file name',
        );
      },
    );
  });
  ...
  expect.test('::prepareJSONBlobAsync', (expect) => {
    return Downloader.prepareJSONBlobAsync('abcd', 'newfile.json').then(
      (dataObject) => {
        const blob = dataObject.blob;
        expect.equal(
          blob.type,
          'application/json',
          'Should encode data as json',
        );
        expect.equal(
          dataObject.name,
          'newfile.json',
  ...
  expect.test('::prepareCanvasBlobAsync', (expect) => {
    return Downloader.prepareCanvasBlobAsync(canvas, 'newfile.png').then(
      (dataObject) => {
        const blob = dataObject.blob;
