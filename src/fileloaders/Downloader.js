...
export default class Downloader {
  ...
  static asFiles(asyncFileArr /* [{name: ..., blob: ...}] */) {
    asyncFileArr.forEach((promise) =>
      promise.then((blobObj) => FileSaver.saveAs(blobObj.blob, blobObj.name)),
    );
  }

  static asZIP(
    asyncFileArr /* [{name: ..., blob: ...}] */,
    name = 'myZip.zip',
  ) {
    const zip = new JSZip();

    Promise.all(asyncFileArr).then(() =>
      zip
        .generateAsync(
          {
            type: 'blob',
    ...
    asyncFileArr.forEach((promise) =>
      promise.then((blobObj) =>
        zip.file(blobObj.name, blobObj.blob, {binary: true}),
      ),
    );
  }
}
