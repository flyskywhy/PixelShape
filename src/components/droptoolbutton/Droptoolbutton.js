...
class DropToolButton extends Component {
  constructor(props) {
    ...
    this.dropdownLength =
      this.props.dropdownTools && this.props.dropdownTools.length;
    this.toolWidth = 70;
  }
  ...
  componentDidMount() {
    if (this.props.dropdownTools) {
      this.props.dropdownTools.forEach((toolObj) => {
        this.bindKey(
          toolHotkeys[toolObj.tool],
          this.setActiveTool.bind(this, toolObj),
        );
      });
    } else {
      this.bindKey(
        toolHotkeys[this.props.tool],
        this.props.setTool.bind(null, this.props.tool),
      );
    }
  }
  ...
  getActiveTool() {
    const classes = classNames('toolbutton__active-button', {
    ...
    return (
      <div
        className={classes}
        onClick={this.props.setTool.bind(null, this.state.activeTool)}
        data-tooltip={this.props.name}
        data-shortcut={`(${toolHotkeys[this.props.tool]})`}>
        <svg
          className="toolbutton__icon"
          viewBox="0 0 24 24"
          width="40"
          height="40">
  ...
  getDropdownTools() {
    return this.props.dropdownTools.map((toolObj, key) => {
      ...
      return (
        <li
          ...
          <svg
            className="toolbutton-dropdown__toolbutton__icon"
            viewBox="0 0 24 24"
            width="40"
            height="40">
  ...
  render() {
    const classes = classNames('toolbutton', 'tooltip-right', {
