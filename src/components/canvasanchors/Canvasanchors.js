...
class CanvasAnchors extends Component {
  constructor(...args) {
    super(...args);
    this.state = {
      anchorState: this.setCurrentAnchorState(
        ...this.getShiftsFromAnchor(this.props.anchor),
      ),
    };
  }
  ...
  setCurrentAnchorState(verticalShift, horizontalShift) {
    ...
    for (i = 0; i < 3; i++) {
      for (j = 0; j < 3; j++) {
        ...
        if (indRow < 0 || indRow >= 3 || indCol < 0 || indCol >= 3) {
          continue;
        }
  ...
  updateAnchorState(verticalShift, horizontalShift) {
    const normalized = this.normalizeAnchorShifts(
        verticalShift,
        horizontalShift,
      ),
      ...
      <ul className={`canvas-anchors-group canvas-anchors-group-${ident}`}>
        {idents.map((id, i) => (
          <li
            ...
            key={i}
            onClick={() => this.updateAnchorState(row, i)}>
            <svg
              className="canvas-anchor__icon"
              viewBox="0 0 32 32"
              width={24}
              height={24}>
  ...
  render() {
    const classes = classNames('canvas-anchors', this.props.className, {
