...
export class StateConverter {
  static hydrateOnImport(fromObj, toObj, schemaType) {
    Object.keys(schemaType).forEach((path) => {
  ...
  static convertToExport(stateData) {
    ...
    // attach metadata
    Object.keys(SerializationSchema.meta).forEach((key) => {
      converted.meta[key] = SerializationSchema.meta[key];
    });

    // attach app data
    Object.keys(SerializationSchema._export).forEach((key) => {
      converted.app[key] = StateSelector[SerializationSchema._export[key]](
        stateClone,
      );
    });
  ...
  static convertToImport(stateObj) {
    ...
    StateConverter.hydrateOnImport(
      stateObj.app,
      converted,
      SerializationSchema._import,
    );
    ...
    getFrames(converted).order.modifiedFramesArray = getFrames(
      converted,
    ).order.framesOrderArray.map((el, key) => ({[el]: key}));
    ...
    nums = getFrames(converted).order.framesOrderArray.map(
      (el) => +el.match(/\d+$/)[0],
    );
  ...
  static createStateFromFramesData(frames, fps, width, height) {
    ...
    StateConverter.hydrateOnImport(
      surrogate,
      converted,
      SerializationSchema._framesImport,
    );
