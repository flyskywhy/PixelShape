...
export const getContextColor = (context, x, y) =>
  context.getImageData(x | 0, y | 0, 1, 1).data;
...
export const equallyColored = (imageData, pixelIndex, color) => {
  ...
  if (pixelIndex < 0 || pixelIndex + RGBA - 1 > imageData.data.length) {
    return false;
  }
...
export const getBlackWhiteContrastColor = (color) =>
  isLightColor(color) ? '#000000' : '#ffffff';

export const getImageDataActiveColors = (
  imageData,
  transparent = [0, 0, 0, 0],
) => {
  const list = [];
  ...
  for (i = 0; i < width; i++) {
    for (j = 0; j < height; j++) {
      ...
      if (colorsEqual(color, transparent) || list.includes(hex)) {
        continue;
      }
      list.push(hex);
    }
...
export const getAllActiveColors = (
  imageDataArr,
  transparent = [0, 0, 0, 0],
) => {
...
export const replaceColor = (imageData, baseColor, replacementColor) => {
  ...
  for (i = 0; i < width; i++) {
    for (j = 0; j < height; j++) {
      ...
      if (equallyColored(imageData, index, baseColor)) {
        putColor(imageData, index, replacementColor);
      }
    }
  }
};
